...
struct RDKIT_MOLDRAW2D_EXPORT MolDrawOptions {
  ...
  double multipleBondOffset = 0.15;  // offset (in Angstrom) for the extra lines
                                     // in a multiple bond
  double multipleBondTruncation = 0.15;  // the percent shorter that the extra bonds
                                     // in a double bond are
  double padding =
...
class RDKIT_MOLDRAW2D_EXPORT MolDraw2D {
  ...
  */
  ...
  Point2D bottomLeft() const { return Point2D(bbox_[0].x, bbox_[0].y); }
  Point2D topRight() const { return Point2D(bbox_[1].x, bbox_[1].y); }
